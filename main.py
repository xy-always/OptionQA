...
from data_util import MedQA_dataset
...
class GeneratorModel:
    ...
    def run(self, question_str, options):
        ...
        generator_prompt += f'Options: {options}\n'
...
class Experiment:
    def __init__(self, data_dir, generator_model_name_or_path, generator_type, evaluator_model_name_or_path, device, max_context_window, generator_model=None):
        ...
        self.benchmark = Benchmark(self.data_dir, 'medqa')

        if generator_model == None:
            self.generator_model = GeneratorModel(model_name_or_path=self.generator_model_name_or_path, generator_type=self.generator_type, device=self.device, max_context_window=self.max_context_window)
        else:
...
def main():
    ...
    responses, evaluation_results = experiment.run()
