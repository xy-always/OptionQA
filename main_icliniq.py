...
from icliniq_data import icliniq
...
class Benchmark:
    def __init__(self, data_dir, dataname='icliniq'):
        ...
        self.test_data = icliniq(dataname, data_dir).test_data()

    def generate_responses_for_question(self, generator_model, question_str, answer_icliniq):
        generator_response_str = generator_model.run(question_str)
...
class Experiment:
    def __init__(self, data_dir, generator_model_name_or_path, generator_type, evaluator_model_name_or_path, device, max_context_window, dataset_name, output_file, generator_model=None):
        ...
        if generator_model == None:
            ...
            self.generator_model = GeneratorModel(model_name_or_path=self.generator_model_name_or_path, generator_type=self.generator_type, device=self.device, max_context_window=self.max_context_window, system_prompt=None)
        else:
